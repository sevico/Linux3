...
getname_flags(const char __user *filename, int flags, int *empty)
{
	...
	if (result)
		return result;
	//在内核缓冲区专用队列里申请一块内存用来放置路径名，其实这块内存就是一个 4KB 的内存页
	result = __getname();
	if (unlikely(!result))
	 ...
	 * allocation
	 */
	//初始化字符串指针 kname，使其指向这个字符串的首地址，相当于 kname = (char *)((struct filename *)result + 1)
			...
			bool got_write, int *opened)
{
	...
	*opened &= ~FILE_CREATED;
	...
	if (!dentry->d_inode && (op->open_flag & O_CREAT)) {
		...
		//如果该文件的确不存在的话会被重新赋上 FILE_CREATED，表示这个文件是这次创建的
		*opened |= FILE_CREATED;
		...
		const struct open_flags *op)
{
	...
	if (unlikely(filp == ERR_PTR(-ESTALE)))//该文件已经过期（stale）才启动
